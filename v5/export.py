...
import onnx, os
import numpy as np
from model_v5 import MTClsModel
...
def export(model, save_path):
    ...
    input_data = torch.randn(1, 3, 30, 22, device=device)

    # 转化为onnx模型
    input_names = ['input']
    output_names = ['output']
    torch.onnx.export(model, input_data, save_path, training=torch.onnx.TrainingMode.EVAL, opset_version=9, verbose=True,
...
def vis_onnx():
    onnx_path = '../onnx_model/' + file_name + '/' + file_name + '.onnx'
    model = onnx.load_model(onnx_path)
    _model = torch.load(model_path)
    graph = model.graph
    print(graph.node[3].input)
    for i in graph.node:
        if "conv1.bn.bias" in i.input:
            i.input[2] = "bottle_neck1.bottle_neck.2.bn.bias"
            # print(i.name)
        if "conv2.bn.bias" in i.input:
            i.input[2] = "bottle_neck2.bottle_neck.2.bn.bias"
            # print(i.name)
        if "conv3.bn.bias" in i.input:
            i.input[2] = "bottle_neck3.bottle_neck.2.bn.bias"
            # print(i.name)
    ...
    print(model)
    print(_model)
    onnx.save_model(model, '../onnx_model/2023_4_21_fix_num_2.onnx')
